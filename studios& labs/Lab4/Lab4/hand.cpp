...
// print all the card information ignore they are face down 
...
// print the cards that only face up
...
hand hand::final_5_hand() {
	hand final_one;
	if (this->hand_.size()!=7) {
		cout << "you don't regular 7 CARDS in hand, don't do anything" << endl;
		return *this;
	}
	else  {
		...
		cout << " : original seven cards are : " << temp1.asString() << endl;
		cout << "print all possible 5 cards hand" << endl;
		for (size_t i = 0; i < 6;i++) {
			...
			for (size_t j = i; j < 6; j++) {
				temp2 = temp1;
				temp2.remove_card(j);
				cout << temp2.asString() << endl;
				all_hands.push_back(temp2);
			}
		}
		//cout << all_hands.size() << " should be 21" << endl;
		sort(all_hands.begin(),all_hands.end(), hand::poker_rank);
		final_one = all_hands[0];
	}
