...
bool SevenCardStud::AllisNum(string str)
{
...
// the card_number to give is based on how many cards the player has right now

int SevenCardStud::turn(player &p) {

	if (p.player_hand.size() == 7) {
		//cout << "| " << p.get_name() << " | " << p.player_hand << endl;
		return card1::success_fail::success;
	}
	int card_number_need = 7 - p.player_hand.size();
	...
	while (p.player_hand.size() != 7) {
		if (deck_game.size() != 0) {
			deck_game.shuffle();// shuffle the deck
...
//
int SevenCardStud::before_turn_1(player &p) {
	//lab4 
	if (p.is_fold == true) {
		cout << "| " << p.get_name() << " | " << p.player_hand << endl;
		cout << "chose to fold, let's drop him and goto next player" << endl;
...
int SevenCardStud::before_round() {
	...
	int first_turn_face_up_card_number = 2;
	int first_turn_face_down_card_number = 1;
	// deal the face up card
	for (size_t i = 0; i < players_game.size() * first_turn_face_up_card_number; i++) { // 2* players number

		player &p1 = *players_game[player_right_now];
		if (p1.player_hand.size() == 2) {
			player_right_now++;
		...
		temp << deck_game;
		temp.face_up = true;
		p1.player_hand.hand_.push_back(temp);
		player_right_now++;
		player_right_now = player_right_now % players_game.size();
	...
	for (size_t i = 0; i < players_game.size() * first_turn_face_down_card_number; i++) { // 1* players number

		player &p1 = *players_game[player_right_now];
		if (p1.player_hand.size() == 3) {
			player_right_now++;
			player_right_now = player_right_now % players_game.size();
			continue;
		}
	...
	cout << "their initial 3 cards" << endl;
	for (size_t i = 0; i < players_game.size(); i++) {
		cout << "Name : " << (*players_game[i]).get_name() << "| hand : " << (*players_game[i]).player_hand << endl;
	}
...
int SevenCardStud::middle_round1() {
	

	cout << "___________"<< "round number is : " << 1 <<"___________" << endl;
	deck_game.shuffle();
	size_t start_index = position_dealer + 1;
	//or, if the player in the last position is the dealer, starting at position 0),
	start_index = start_index % players_game.size();
	...
	for (size_t i = 0; i < players_game.size() * to_deal; i++) { // 1* players number

		player &p1 = *players_game[player_right_now];
		if (p1.player_hand.size() == 4) {
			player_right_now++;
			player_right_now = player_right_now % players_game.size();
			continue;
		}
	...
	cout << "their initial "<< (4) <<" cards" << endl;
	for (size_t i = 0; i < players_game.size(); i++) {
		cout << "Name : " << (*players_game[i]).get_name() << "| hand : " << (*players_game[i]).player_hand << endl;
	}
	start_index = position_dealer + 1;
	//or, if the player in the last position is the dealer, starting at position 0),
	start_index = start_index % players_game.size();
	player_right_now = start_index;

	cout << "after the "<< 1 <<" th betting phase, the total amount in game pot is : " << pot_amout << endl;
	highest_bet_amount = 0;// every one need to bet
	phase_done = false;// all but one choose to fold
	phase_done_one_round = false;
	int check_round = 0;
	bool already_one_round = false;
	//highest_bet_amout = 0;// every one need to bet
	bet_player_number = 0;
	for (size_t i = 0; i < players_game.size(); i++) {
		(*players_game[i]).bet_total = 0;
	}
...
int SevenCardStud::middle_round2() {
	

	cout << "___________" << "round number is : " << 2 << "___________" << endl;
	deck_game.shuffle();
	size_t start_index = position_dealer + 1;
	//or, if the player in the last position is the dealer, starting at position 0),
	start_index = start_index % players_game.size();
	...
	cout << "after the " << 2 << " th betting phase, the total amount in game pot is : " << pot_amout << endl;
	highest_bet_amount = 0;// every one need to bet
	phase_done = false;// all but one choose to fold
	phase_done_one_round = false;
	int check_round = 0;
...
int SevenCardStud::middle_round3() {
	

	cout << "___________" << "round number is : " <<3<< "___________" << endl;
	deck_game.shuffle();
	size_t start_index = position_dealer + 1;
	//or, if the player in the last position is the dealer, starting at position 0),
	start_index = start_index % players_game.size();
	...
	cout << "their initial " << 6 << " cards" << endl;
	for (size_t i = 0; i < players_game.size(); i++) {
		cout << "Name : " << (*players_game[i]).get_name() << "| hand : " << (*players_game[i]).player_hand << endl;
	}
	start_index = position_dealer + 1;
	...
	cout << "after the " << 3<< " th betting phase, the total amount in game pot is : " << pot_amout << endl;
	highest_bet_amount = 0;// every one need to bet
	phase_done = false;// all but one choose to fold
	phase_done_one_round = false;
	int check_round = 0;
...
int SevenCardStud::last_round() {
	
	cout << "___________" << "turn number is : Last Turn" << "___________" << endl;
	deck_game.shuffle();
	size_t start_index = position_dealer + 1;
	//or, if the player in the last position is the dealer, starting at position 0),
	start_index = start_index % players_game.size();
	...
	cout << "after the 4" << " th betting phase, the total amount in game pot is : " << pot_amout << endl;
	highest_bet_amount = 0;// every one need to bet
	phase_done = false;// all but one choose to fold
	phase_done_one_round = false;
	int check_round = 0;
...
int SevenCardStud::round() {
	...
	*/
	...
	cout << endl << "middle_round()----" << 1 << "--------------" << endl;
	middle_round1();
	cout << endl << "middle_round()----" << 2 << "--------------" << endl;
	middle_round2();
	cout << endl << "middle_round()----" << 3 << "--------------" << endl;
	middle_round3();
	
	cout << endl << "-last_tound()------------------" << endl;
	last_round();
...
int SevenCardStud::after_round() {
	...
	cout << "_________________________" << endl;
	cout << "_________________________" << endl;
	cout << "all the pot in the pot pool is : " << pot_amout << endl;
	cout << "let the dealer show all the cards in players' hand after using final_5_hand" << endl;
	vector <shared_ptr<player>> players_temp(players_game);

	for (size_t i = 0; i < players_temp.size(); i++) {
		cout << endl;
		cout << players_temp[i]->get_name();
		sort(players_temp[i]->player_hand.hand_.begin(), players_temp[i]->player_hand.hand_.end());
		players_temp[i]->player_hand = players_temp[i]->player_hand.final_5_hand();
		cout << "Name : " << players_temp[i]->get_name() << " | Hand : " << players_temp[i]->player_hand.asString() << endl;
	}


	
	sort(players_temp.begin(), players_temp.end(), poker_rank1);
...
//lab 4
int SevenCardStud::bet_phase(player & p1) {
	...
	if (bet_player_number == 0) {
		...
		// player has no money
		if (p1.chips <= 0 || p1.chips-1> p1.chips) {
		...
		while (ai == 0) {
			...
			if (input12 == "check") {
				...
				cout << "you do a wise choice ----- check" << endl;
				ai = 1;
	...
	// some players already bet 
		...
		cout << "someone already bet, so you have to call or fold or raise" << endl;
		cout << "you have to bet at least : " << ((int)(highest_bet_amount) - (int)(p1.bet_total)) << endl;//
		cout << "you at most can bet : " << ((int)(highest_bet_amount)-(int)(p1.bet_total)) + 2 << endl;
		cout << "let's first check if you have enough money" << endl;
		//check if  this player has money
		cout << "highest_bet_amout" << highest_bet_amount << endl;
